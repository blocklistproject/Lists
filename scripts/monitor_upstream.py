...
def create_pr_branch(list_name: str, results: list[UpdateResult]) -> bool:
    ...
    try:
        ...
        try:
            subprocess.run(
                ["node", str(scripts_dir / "generate-noip.js")],
                cwd=PROJECT_ROOT,
                check=True,
                capture_output=True
            )
            subprocess.run(
                ["node", str(scripts_dir / "generate-dnsmasq.js")],
                cwd=PROJECT_ROOT,
                check=True,
                capture_output=True
            )
            subprocess.run(
                ["node", str(scripts_dir / "generate-adguard.js")],
                cwd=PROJECT_ROOT,
                check=True,
                capture_output=True
            )
            ...
            print(f"      Output: {e.stderr.decode() if e.stderr else 'none'}")
...
def main():
    ...
    if not args.dry_run and total_new > 0:
        ...
        for list_name, results in all_results.items():
            ...
            if create_pr_branch(list_name, results):
