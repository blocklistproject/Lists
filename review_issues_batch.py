...
VAULT_DIR = Path(os.environ.get("HERMES_VAULT", "/home/administrator/.hermes/vault"))
WORKSPACE = Path("/home/administrator/.hermes/workspace/Lists")
LOCK_DIR = Path("/tmp/blocklistproject-triage-cron.lock")
ISSUES_FILE = Path("/tmp/issues.json")
RESULTS_FILE = Path("/tmp/batch_results.json")
...
def parse_issue(issue: dict) -> dict:
    ...
    if "[add" in title.lower():
        ...
        # Extract list from title or body
